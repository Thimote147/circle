...
export const GroupIssues: FC<GroupIssuesProps> = ({ status, issues, count }) => {
   ...
   useEffect(() => {
      const fetchSortedIssues = async () => {
         const sorted = await sortIssuesByPriority(issues);
         ...
         {viewType === 'list' ? (
            <div className="space-y-0">
               {sortedIssues.map((issue) => (
                  <IssueLine key={issue.issue_id} issue={issue} layoutId={true} />
               ))}
            </div>
         ) : (
            <IssueGridList issues={sortedIssues} status={status} />
         )}
      </div>
   );
};
...
const IssueGridList: FC<{ issues: Issue[]; status: Status }> = ({ issues, status }) => {
   ...
   const [{ isOver }, drop] = useDrop(() => ({
      ...
      drop(item: Issue, monitor) {
         if (monitor.didDrop() && item.status.status_id !== status.status_id) {
            updateIssueStatus(item.issue_id, status);
         }
      },
      collect: (monitor) => ({
